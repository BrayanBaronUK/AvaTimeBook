...
export class IndividualService {
  ...
  Gruposmeesages(value){ //aceder a los mensajes de personas
    ...
    .collection('SeguidorPersona').doc(value).collection('messages', ref => 
                                                  ref.orderBy('fecha','desc'));
    return this.itemsCollectionmensajes.valueChanges()
                                .map( (mensajes:Mensaje[]) =>{
  ...
  GrupoagregarMensaje(value, texto:string ){
    ...
    this.afs2.collection('perfil').doc(value).collection('SeguidorPersona').doc(this.userse.getIud()).
                                          collection("messages").add(mensaje);
    return this.afs2.collection('perfil').doc(this.userse.getIud()).collection('SeguidorPersona').doc(value).
                                          collection("messages").add(mensaje);
